...
//!         Set::<(i32, i32), _>::new_no_generating_set(), // We only test on pairs that are generated by the given maps from the triplets, so no explicit elements are added here
//!         Set::new(vec![(5, 3), (100, 100)]), // We can also insert some additional generating elements into an intermediate set
//!         Set::<i32, _>::new_no_generating_set_checked(|x: &i32| *x >= 0),
//!     ],
//!     vec![
...
impl<T> Set<T, fn(&T) -> bool> where T: Clone + Element + Sized  {
    pub fn new(elements: Vec<T>) -> Rc<Set<T, fn(&T) -> bool>>{
        Rc::new(Self{elements, property: |_x| true})   
    }

    pub fn new_no_generating_set() -> Rc<Set<T, fn(&T) -> bool>> {
        Rc::new(Self {
            elements: Vec::new(), property: |_x| true
        })
    }
}
    ...
    P: Fn(&T) -> bool
{
    ...
    pub fn new_checked(elements: Vec<T>, property: P) -> Rc<Set<T, P>> {
        Rc::new(Self { elements, property})
    }

    pub fn new_no_generating_set_checked(property: P) -> Rc<Set<T, P>> {
        Rc::new(Self {
            elements: Vec::new(), property
        })
    }
}

impl<T, P> SetLike for Set<T, P>
where
...
mod tests {
    ...
    fn test_addition_is_associative_on_integers() {
        ...
        let diagram = Diagram {
            sets: vec![
                Set::new(triplets),
                Set::<(i32, i32), _>::new_no_generating_set(),
                Set::<(i32, i32), _>::new_no_generating_set(),
                Set::<i32, _>::new_no_generating_set_checked(|x: &i32| *x >= 0),
            ],
            maps: vec![
